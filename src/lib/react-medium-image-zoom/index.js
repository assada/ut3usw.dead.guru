...
class ControlledBase extends React.Component {
    constructor() {
        ...
        this.setAndTrackImg = () => {
            ...
            if (!contentEl)
                return;
            const prevImgEl = this.imgEl;
            const nextImgEl = contentEl.querySelector(IMAGE_QUERY);
            if (nextImgEl && prevImgEl && nextImgEl !== prevImgEl) {
                this.imgElResizeObserver?.disconnect?.();
                prevImgEl.removeEventListener('load', this.handleImgLoad);
                prevImgEl.removeEventListener('click', this.handleZoom);
            }
            this.imgEl = nextImgEl;
            if (this.imgEl) {
                ...
                this.imgElResizeObserver.observe(this.imgEl, { attributes: true , subtree: true});

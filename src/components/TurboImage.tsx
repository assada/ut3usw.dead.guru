...
const TurboImage: React.FC<TurboImageProps> = ({ img, zoomSrc, alt, title, width, height }) => {
    ...
    return (
        <Zoom swipeToUnzoomThreshold={5} zoomMargin={20} zoomImg={{ src: zoomSrc || img.src.images[img.src.images.length - 1].path }}>
            <img loading="lazy" decoding="async" srcSet={img.src.srcSet} className="medium-zoom-image" src={img.src.images[0].path} alt={alt} width={width || img.src.images[0].width} height={height || img.src.images[0].height} />
        </Zoom>
    );
};

...
import codeExamplesData from '@site/src/generated/code-examples.json';


function RandomCodeExample() {
  const [index, setIndex] = useState(0);

  useEffect(() => {
    setIndex(Math.floor(Math.random() * codeExamplesData.length));
  }, []);

  return (
    <div
      className="shiki-inline"
      dangerouslySetInnerHTML={{ __html: codeExamplesData[index].html }}
    />
  );
}
